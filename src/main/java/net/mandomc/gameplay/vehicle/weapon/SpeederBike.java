...
public class SpeederBike implements WeaponSystem {
    ...
    public void shoot(Vehicle vehicle, Player shooter) {
        ...
        debug("SpeederBike shoot requested by " + shooter.getName() + " vehicle=" + vehicleId);
    ...
    private void fireWeapon(Vehicle vehicle, Player shooter, ConfigurationSection config) {
        ...
        if (!AmmoUtil.hasAmmo(shooter, ammo, ammoPerShot)) {
            ...
            shooter.sendMessage(LangManager.get("vehicles.weapon.out-of-ammo", "%ammo%", (ammo != null ? ammo.replace("_", " ") : "ammo")));
            return;
        }
